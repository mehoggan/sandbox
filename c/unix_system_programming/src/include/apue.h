...
int send_err(int, int, const char *);                                          /* Figures 17.19 */
int serv_listen(const char *);                                                 /* Figures 17.10 and 17.15 */
int serv_accept(int, uid_t *);                                                 /* Figures 17.11 and 17.16 */
int cli_conn(const char *);                                                    /* Figures 17.12 and !7.17 */
int buf_args(char *, int (*funct)(int, char **));                              /* Figures 17.32 */

int tty_cbreak(int);                                                           /* Figures 18.20 */
int tty_raw(int);                                                              /* Figures 18.20 */
int tty_reset(int);                                                            /* Figures 18.20 */
void tty_atexit(void);                                                         /* Figures 18.20 */
struct termios *tty_termios(void);                                             /* Figures 18.20 */

int ptym_open(char *, int);                                                    /* Figures 19.8, 19.9, and 19.10 */
int ptys_open(char *);                                                         /* Figures 19.8, 19.9, and 19.10 */

#ifdef TIOCGWINSZ
pid_t pty_fork(int *, char *, int, const struct termios *,                     /* Figure 19.11 */
