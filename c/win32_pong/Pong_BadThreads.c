...
DWORD WINAPI LeftPaddleProc(PVOID pParam)
{
    MessageBox(NULL, TEXT("Left Thread Called"), TEXT("Thread Called"), 0);
    switch((TCHAR) pParam)
    {
        case 'w':
        {
            ...
            {
                MessageBox(NULL, TEXT("w Pressed"), TEXT("Thread Called"), 0);
                corners1.left = 0;
                corners1.right = 20;
                corners1.top = rect1[0].y;
                corners1.bottom = rect1[1].y;
                InvalidateRect(h_MainWindow, &corners1, TRUE);
        ...
        case's':
        {
            ...
            {
                MessageBox(NULL, TEXT("s Pressed"), TEXT("Thread Called"), 0);
                corners1.left = 0;
                corners1.right = 20;
                corners1.top = rect1[0].y;
                corners1.bottom = rect1[1].y;
        ...
        default:
        {
            MessageBox(NULL, TEXT("No Button Pressed"), TEXT("Thread Called"), 0);
        }
    }
    return 0;
}

DWORD WINAPI RightPaddleProc(PVOID pParam)
{
    MessageBox(NULL, TEXT("Right Thread Called"), TEXT("Thread Called"), 0);
    switch((TCHAR)pParam)
    {
        case'o':
        {
        ...
        case 'l':
        {
            ...
            {
                MessageBox(NULL, TEXT("l Pressed"), TEXT("Thread Called"), 0);
                corners2.left = rect.right - 20;
                corners2.right = rect.right;
                corners2.top = rect2[0].y;
                corners2.bottom = rect2[1].y;
    ...
    switch (message)
    {
        ...
        case WM_CHAR:
        {
            switch((TCHAR)wParam)
            {
                case 'w':
                {
                    hLeftThread = CreateThread(NULL, 0, &LeftPaddleProc, &wParam, 0, NULL);
                    return 0;
                }
                case 's':
                {
                ...
                case 'o':
                {
                    hRightThread = CreateThread(NULL, 0, &RightPaddleProc, &wParam, 0, NULL);
                    return 0;
                }
                case '1':
                {
