namespace glext
{
  ...
  <
    ...
    render_target_enum render_target
  >
  void batch_interleaved_renderer
  <
    render_data_type,
    render_primitive_mode,
    render_usage,
  ...
  <
    ...
    render_target_enum render_target
  >
  bool batch_interleaved_renderer
  <
    render_data_type,
    render_primitive_mode,
    render_usage,
  ...
  >::load_data(render_data_type &data, GLsizei buffer_count)
  {
    ...
    _vertex_count = data._data.size();
    _byte_count = data.size_of();

    glGenBuffers(buffer_count, &_vbo_id);
    if (bind()) {
      glBufferData(render_target, _byte_count, data._data.data(), render_usage);
    } else {
      return false;
    }
