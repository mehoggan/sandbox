...
void TimerManager::removeTimer(long usec, void (*callback)(int id)) {
  pthread_mutex_lock(&m_tGoLock);
  Timer l_oTimer(usec, callback);

  std::list<Timer>::iterator new_end = std::remove_if(
              m_cTimers.begin(),
              m_cTimers.end(),
              TimerManager::Timer(l_oTimer)
             );
