...
; Description      : A simple program in assembly for Linuxa, using
...
SCRWIDTH:  equ 0x50                                                            ; by default we assume 80 chars wide
...
; calculate ASCII digits to insert in the terinal contorl string,
...
;-----------------------------------------------------------------------------
; GotoXY:      Postion the Linux Console Cursor to an X,Y position
; UPDATED:     4/21/2009
; IN:          X in AH, Y in AL
; RETURNS      NOTHING
...
; Send control sequence to stdout:
  ...
  mov edx, POSLEN                                                              ; Pass length of the control string
...
WriteCtr:
  ...
  shr bl, 0x01                                                                 ; Divide difference by two for X value
  mov ah, bl                                                                   ; GotoXY requires X value in AH
  call GotoXY                                                                  ; Position the cursor for the display
...
Exit:
  ...
  mov ebx, 0x00                                                                ; Return a code of 0
  int 0x80                                                                     ; Make kernel call
