...
;                  sequence to do simple "full-screen" text output
;                  through macros rather than procedures
;
; Building using these commands:
...
SCRWIDTH:  equ 0x05                                                            ; By default we assume 80 chars wide
PosTerm:   db 27,"[01:02H"                                                     ; <ESC>[<Y>;<X>H
...
; calculate ASCII digits to insert in the terminal contorl string,
...
; MODIFIES:    Nothing
; CALLS:       Nothing
; DESCRIPTION: Call sys_edit to terminate the program and return
;              control to Linux
%macro ExitProg 0
...
;              the console 
%macro WaitEnter 0
  mov eax, 0x03                                                                ; Code for sys_read
  mov ebx, 0x00                                                                ; Specify File Descriptor 0: STDIN
  int 0x80                                                                     ; make kernel call
%endmacro
...
%macro WriteStr 2                                                              ; %1 = String address, string length in %2
...
; MODIFIES:    Nothing
; CALLS:       Kernel sys_write
; DESCRIPTION: Sends the predefined contorl string <ESC>[2J to the
...
%macro ClrScr 0
  ...
  push ecx
  push edx
; User WriteStr macro to write contorl string to console:
...
;              console cursor to the X,Y position. Writing text to the
...
; RETURNS:     Nothing
; MODIFIES:    PosTerm terminal contorl sequence string
...
%macro WriteCtr 3                                                              ; %1 = row; %2 = String addr; %3 = String length
  ...
  sub bl, dl                                                                   ; Calculate diff. of screen width and string length
  shr bl, 0x01                                                                 ; Divide difference by two fro X value
